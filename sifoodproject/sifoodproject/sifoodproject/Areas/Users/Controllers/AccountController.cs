...
namespace sifoodproject.Areas.Users.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            string? account = _context.Users.Where(x => x.UserEmail == model.EmailAccount).Select(y => y.UserEmail).FirstOrDefault();
            string? userName = _context.Users.Where(x => x.UserName == model.UserName).Select(y => y.UserName).FirstOrDefault();
            if (account != null)
            {
                return "此帳號已被註冊";
            }
